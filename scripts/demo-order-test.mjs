 ...
 *
 * One-off DEMO mechanics test: places a single 1-contract YES FOK order at 1¢ on
 * the current Kalshi BTC 15m demo market. At 1¢ it WON'T fill (nothing sells that
 * low), FOK auto-cancels → no position created, balance untouched. The point is
 * purely to prove the signed POST /portfolio/orders round-trips (auth + request
...
try {
  ...
  log(`Enviando ordem FOK: 1x YES @ 1¢ (client_order_id=${clientOrderId})`);
  log("--- (a 1¢ não deve preencher; FOK cancela sozinho) ---");
  ...
  log("\n=== PÓS-TESTE ===");
  log(`Saldo demo DEPOIS: $${balAfter.toFixed(2)} (mudou? ${balBefore !== balAfter ? "SIM ⚠️" : "não ✅"})`);
  log(`Posição no ticker: ${pos ? JSON.stringify(pos) : "nenhuma ✅"}`);

  log("\n=== VEREDITO ===");
  log("✅ MECÂNICA OK: a API aceitou o POST assinado (auth + formato corretos).");
  if (res.filled) log("⚠️ Preencheu inesperadamente — verificar (não deveria a 1¢).");
} catch (e) {
  log("\n=== ERRO ===");
